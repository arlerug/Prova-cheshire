...
EMB_DIR    = Path("data_embeddings_e5")
...
def main():
    ...
    with open(manifest_out, "w", encoding="utf-8") as mf_out:
        for f in tqdm(files, desc="Embeddings e5-large"):
            ...
            with torch.no_grad():
                out = model(**inputs)
                # media sui token embeddings (sentence embedding standard)
